	...
	<Card class="auth-card">
		<Stack gap={5}>
			...
			<p class="auth-switch">
				{#if flow === 'signIn'}
					New here?
					<button type="button" class="auth-link" onclick={() => navigate('register')}>
						Create an account
					</button>
				{:else}
					Already have an account?
					<button type="button" class="auth-link" onclick={() => navigate('login')}>
						Sign in
					</button>
				{/if}
			</p>
...
<style>
	.auth-page {
		...
		/* Respect notch/home-indicator on all four edges. */
		padding: max(var(--sv-space-4), var(--sv-safe-top)) max(var(--sv-space-4), var(--sv-safe-right))
			max(var(--sv-space-4), var(--sv-safe-bottom)) max(var(--sv-space-4), var(--sv-safe-left));
	}
