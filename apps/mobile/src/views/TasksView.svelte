			...
			<Container size="sm">
				<Stack gap={5}>
					...
					{:else if sorted.length === 0}
						<EmptyState
							title="No tasks yet"
							description="Add your first task above — it syncs live with web and desktop."
						>
...
<style>
	.shell-header {
		...
		/* Extend under the status bar / notch. */
	...
	.shell-main {
		...
		padding: var(--sv-space-4) 0 calc(var(--sv-space-16) + var(--sv-safe-bottom));
	}
	.task-form__row {
		display: flex;
		gap: var(--sv-space-2);
	...
	.task-item {
		...
		align-items: center;
		gap: var(--sv-space-3);
		/* ≥44px rows via --sv-touch-target from platform/mobile.css */
